...
const char* MyPluginExtensionName = "ADBE:Check E-pubs";
...
ACCB1 void ACCB2 MyPluginCommand(void *clientData)
{
	...
	if(avDoc==NULL) {
		...
		ASText temp = ASTextNew();
		ASTextSetPDText(temp,"CreationDate");
		PDDocGetInfoASText(pdDoc,temp, creationDate);
		ASTextSetPDText(temp, "Title");
		PDDocGetInfoASText(pdDoc, temp, fileName);
		//sprintf(str,"%sThe active PDF document has %d pages.", str, numPages);
		sprintf(str2, "Hello, I am: %s:\nAnd I was created on %s: ", ASTextGetEncodedCopy(fileName, 0), ASTextGetEncodedCopy(creationDate, 0));
		AVAlertNote(str2);
